...
macro "Alex's Kinetochore Measurer" {
...
//Dialog to define inputs
	...
	if(isOpen("Log")){
 	...
 	filter="D3D.dv";
	Dialog.addNumber("Subunit",1);
	Dialog.addNumber("MTs",2);
	Dialog.addNumber("DAPI",3);
	Dialog.addNumber("ACA",4);
	Dialog.addString("Suffix Filter :", filter);
	Dialog.addNumber("# of Objects: ", 20);
	Dialog.addCheckbox("Batch Mode?", true) 	

	Dialog.show();

	subunit=Dialog.getNumber();
	MTs=Dialog.getNumber();
	DAPI=Dialog.getNumber();
	aca=Dialog.getNumber();
	filter=Dialog.getString();
	objects=Dialog.getNumber();
	batcher=Dialog.getCheckbox();
...
//get info for the open image
list=getTitle;
subdir=getDirectory("Choose where to save results");
length=1;
...
//process each image, only with "*.filter" suffix
     if (endsWith(list[i],filter)) {
		...
		//import image, get stack dimensions, make composite projection for identifying mitotic and interphase cells     	
...
//show all cell selections and show dialog to identify celltype for each, continue by OK
	selectWindow("Image Map of "+list[i]);
	celltype=newArray("mitotic","interphase");
	selectWindow("Image Map of "+list[i]);
...
//reopen ROI file if closed, select image, select cell by ROI, duplicate this cells' hyperstack (all channels), duplicate subunit channel to regular stack (single channel)
...
//count = keep track of the cells ROI in ROI manager
	...
	for(count=0;count < maxCell;count++){
		...
		rename(cellHyperStack);

		//duplicate subunit channel for kinetochore identification
		Stack.setChannel(subunit);
		reduceHyperStack(subunit);
		Stack.getDimensions(width, height, channels, slices, frames);
		...
		//go back to hyperstack of this cell, duplicate subunit channel stack, subtract background and gather pixel values 
		selectWindow(cellHyperStack);
		Stack.setChannel(subunit);
		reduceHyperStack(subunit);
		...
		for(resultNumber;resultNumber<max;resultNumber++){
			appendMax(getResult("x", resultNumber),getResult("y", resultNumber),getResult("z", resultNumber),resultNumber,"subtracted subunit");
			}
		close();
		
		//go back to hyperstack of this cell, duplicate the aca channel stack and gather pixel values 
		selectWindow(cellHyperStack);
		Stack.setChannel(aca);
		reduceHyperStack(aca);
		Stack.getDimensions(width, height, channels, slices, frames);
		run("Canvas Size...", "width="+width+10+" height="+height+10+" position=Center");

		resultNumber=resultNumber-objects;
		for(resultNumber;resultNumber<max;resultNumber++){
			appendMax(getResult("x", resultNumber),getResult("y", resultNumber),getResult("z", resultNumber),resultNumber,"raw aca");
			}
		close();
		...
		resultNumber=resultNumber-objects;
		for(resultNumber;resultNumber<max;resultNumber++){
			appendMax(getResult("x", resultNumber),getResult("y", resultNumber),getResult("z", resultNumber),resultNumber,"subtracted aca");
			}
...
setSlice(goodslice);
	for(y=1;y<=height;y++){
		for(x=1;x<=width;x++){
			...
			setResult("raw subunit value",resultNumber,getPixel(x,y));
			updateResults();
			}
		}
	}
